...
#define CLUSTERING_TASK_KMEANS 0
...
usage_task_params:
    ...
    if (strcmp(task->sval[0], "kmeans") == 0) {
        return_value = CLUSTERING_TASK_KMEANS;
    } else {
        printf("Unknown clustering task: %s\n\n", task->sval[0]);
        goto usage_task_params;
    }
...
void parse_predict_params(int argc, char *argv[], struct csr_matrix **input_dataset, struct csr_matrix **model, char** path_prediction_result, KEY_TYPE* verbose) {
    struct arg_lit *help = arg_lit0(NULL,"help", "print this help and exit");
    struct arg_int *no_cores = arg_int0(NULL,"no_cores","<no_cores>", "the number of cores to use if compiled with openmp (uses all cores with -1 = default)");
    struct arg_lit *silent = arg_lit0(NULL, "silent", "turn off verbosity (default=false)");
...
void kmeans_task(int argc, char *argv[]) {
    ...
    if (subtask == SUBTASK_PREDICT) {
        ...
        verbose = 1;
        stop = 0;
        parse_predict_params(argc - 1, argv + 1, &input_dataset, &clusters, &path_prediction_file, &verbose);
