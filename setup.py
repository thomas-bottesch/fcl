...
if platform.system() == "Linux":
    ...
    if ldshared is not None:
        os.environ["LDSHARED"] = ldshared.replace(" -g ", " ")
...
import fcl  # still needed for your dynamic version (consumed via pyproject dynamic)


class CleanClass(BaseClean):
    def run(self):
        BaseClean.run(self)
        is_not_sdist = not os.path.exists(
            os.path.join(os.path.dirname(__file__), "PKG-INFO")
        )
        for path, dirs, names in os.walk(
            os.path.join(base_library_path, "python", "fcl")
        ):
            for fname in names:
...
for algo in os.listdir(algorithms_path):
    ...
    if os.path.isdir(algorithms_path_specific):
        ...
        for f in os.listdir(algorithms_path_specific):
            if f.endswith(".c"):
                algorithm_c_files_specific.append(
                    os.path.join(algorithms_path_specific, f)
                )
...
ext_modules = [
    Extension(
        "fcl.kmeans._kmeans",
        [
            os.path.join(base_path, "fcl", "kmeans", "_kmeans.pyx"),
        ...
        + algorithm_c_files["kmeans"],
        extra_compile_args=["-fopenmp", "-DEXTENSION"],
        include_dirs=[".", "python"],
        extra_link_args=["-fopenmp"],
    ),
    Extension(
        "fcl.matrix.csr_matrix",
        [
            os.path.join(base_path, "fcl", "matrix", "csr_matrix.pyx"),
            os.path.join(utils_path, "clogging.c"),
            os.path.join(utils_path, "fcl_logging.c"),
            os.path.join(utils_path, "fcl_file.c"),
            os.path.join(csr_matrix_folder, "csr_load_matrix.c"),
    ...
    Extension(
        "fcl.cython.utils.matrix.csr_matrix.csr_assign",
        [
            os.path.join(
                base_path,
                "fcl",
    ...
    Extension(
        "fcl.cython.utils.cdict",
        [
            os.path.join(base_path, "fcl", "cython", "utils", "cdict.pyx"),
            os.path.join(utils_path, "clogging.c"),
            os.path.join(utils_path, "cdict.c"),
            os.path.join(utils_path, "fcl_logging.c"),
        ],
...
setup(ext_modules=ext_modules, cmdclass={"clean": CleanClass})
